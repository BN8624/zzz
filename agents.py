...
USAGE = {
    ...
    "flash_lite": {"calls": 0, "in": 0, "out": 0},   # 자가발전 교훈정제용(3.1 Flash-Lite)
...
# 자가발전 교훈 정제용 ― 3.1 Flash-Lite 호출.
#  "4점 미만 실패작 → 한 줄 교훈" 압축이라는 한 가지 일만 합니다.
#  무료지만 RPD 가 500 으로 작으니, 빈도 낮은 이 용도에만 씁니다.
#  26B/31B 와 한도가 따로라, 여기 호출은 그 두 모델 페이싱과 무관합니다.
