...
USAGE = {
    ...
    "flash": {"calls": 0, "in": 0, "out": 0},   # 배치 재처리용(3.5 Flash)
...
def _reset_if_new_day():
    ...
    if USAGE["date"] != today:
        ...
        USAGE["flash"] = {"calls": 0, "in": 0, "out": 0}
...
# 배치 재처리(4점 미만 보완)용 ― 3.5 Flash 호출.
#  26B/31B 와 한도가 따로라, 여기 호출은 그 두 모델의 안전선과 무관합니다.
def call_flash(prompt):
    return _call(MODELS["flash"], "flash", prompt)
