...
# 호출수의 "오늘" 기준 + 화면에 보여줄 현재시각은 태평양시(PST/PDT).
#  구글 한도가 태평양시 자정에 리셋되므로, 그 기준의 현재시각을 같이 보여주면
#  "지금 한도 주기에서 얼마나 썼나"를 바로 읽을 수 있습니다.
...
# 대시보드(Flask) 접속 로그를 끕니다.
#  왜? Werkzeug(Flask 내장 서버)가 "GET / HTTP/1.1" 같은 접속 줄을 출력하는데,
#  이게 자동코딩 화면 로그와 같은 곳(output.log)으로 섞여 들어가서
#  흐름 탭 카드 안이 지저분해졌습니다. 접속 로그는 분석에 필요 없으므로 끕니다.
#  (ERROR 급만 남기고, 평범한 접속 줄은 더 이상 찍지 않음)
import logging
...
def get_status():
    ...
    # 오늘(태평양시 기준) 모델별 호출수 + 태평양시 현재시각.
    #  카운터는 agents.USAGE 가 이미 태평양시 자정 리셋으로 세고 있으므로 그대로 읽음.
    usage = get_usage()
    calls_26b = usage.get("26b", {}).get("calls", 0)
    calls_31b = usage.get("31b", {}).get("calls", 0)
    pst_now = datetime.now(_PACIFIC).strftime("%m/%d %H:%M")
...
# 작업 시작을 알리는 줄에 들어있는 표식.
#  ★ 한 작업의 진짜 시작은 "주제 생성"입니다. (topic_generator 의 "...주제 생성 중...")
#    예전엔 "AI AUTO CODER" 박스를 경계로 삼았는데, 그 박스보다 주제실 로그가
#    먼저 찍혀서 → 주제실 부분이 '직전 작업 꼬리'에 딸려 들어가고(잘림),
#    정작 그 작업의 머리(주제실)는 빠지는 문제가 있었습니다.
#    그래서 매 작업마다 반드시 한 번 나오는 "주제 생성 중" 줄을 경계로 바꿉니다.
_CYCLE_MARK = "주제 생성 중"
...
def get_log_cycles():
    ...
    # 1) "주제 생성 중" 줄 위치를 모두 찾습니다(= 각 작업의 시작점).
    ...
    for n, s in enumerate(starts):
        ...
        # 제목 뽑기:
        #  이제 사이클 첫 줄은 "주제 생성 중"(주제실) 입니다.
        #  보기 좋은 제목은 'AI AUTO CODER' 박스 바로 아래의 '실제 주제 문장'이므로,
        #  먼저 그 박스를 찾아 다음 줄을 제목으로 씁니다.
        #  (박스가 없으면 — 첫 줄들 중 글자 있는 줄로 대체)
        title = ""
        mark_at = None
        for k, ln in enumerate(body_lines):
            if "AI AUTO CODER" in ln:
                mark_at = k
                break
        search_from = (mark_at + 1) if mark_at is not None else 1
        for ln in body_lines[search_from:]:
  ...
  <main>
    ...
    <section id="view-status" class="view active">
      <div class="card">
        ...
        <div class="status-sub">태평양 기준 <b id="pst-now">--/-- --:--</b> · 오늘 호출 26B <b id="c-26b">0</b> / 31B <b id="c-31b">0</b></div>
      </div>
      <div class="stat-grid">
        ...
        <div class="stat"><div class="label">26B 호출(오늘·PST)</div><div id="s-26b" class="value">0</div></div>
        <div class="stat"><div class="label">31B 호출(오늘·PST)</div><div id="s-31b" class="value">0</div></div>
      </div>
    </section>
...
async function loadStatus(){
  ...
  // 태평양시 현재시각 + 오늘(PST) 모델별 호출수
  if(d.pst_now){ $('pst-now').textContent = d.pst_now; }
  if(d.calls_26b!==undefined){
    $('c-26b').textContent = d.calls_26b;
    $('c-31b').textContent = d.calls_31b;
    $('s-26b').textContent = d.calls_26b;
    $('s-31b').textContent = d.calls_31b;
  }
