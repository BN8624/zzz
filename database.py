...
from config import DB_PATH, DATA_RETENTION, REPROCESS
...
class CodeDatabase:
    ...
    def init_db(self):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS results (
                ...
                reprocessed_at TEXT               -- 배치 재처리한 시각(없으면 아직 안 함)
            )
        """)
        ...
        # --- 마이그레이션 ---
        # 이미 만들어진 옛 DB에는 reprocessed_at 컬럼이 없습니다.
        # 없으면 추가하고, 이미 있으면 조용히 넘어갑니다(데이터는 그대로).
        try:
            cursor.execute("ALTER TABLE results ADD COLUMN reprocessed_at TEXT")
    ...
    # -------------------------------------------------------------
    #  배치 재처리 ① ― "살릴" 후보 가져오기.
    #   조건: 3점대(min~max) + 아직 재처리 안 함 + 즐겨찾기 아님.
    #   점수 높은 것(거의 4점)부터, 오래된 것부터 골라 돌려줍니다.
    # -------------------------------------------------------------
    def get_reprocess_candidates(self, limit=1):
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute(
            """
            SELECT id, topic, code, ai_score, ai_summary
            FROM results
            ...
            LIMIT ?
            """,
            (REPROCESS["min_score"], REPROCESS["max_score"], limit),
        )
        rows = cursor.fetchall()
        conn.close()
        return [
            {"id": r[0], "topic": r[1], "code": r[2],
             "score": r[3], "summary": r[4]}
            for r in rows
        ]

    # -------------------------------------------------------------
    #  배치 재처리 ② ― 보완 결과를 반영하기.
    #   코드/점수/한줄평을 갱신하고, 새 점수에 맞는 만료일을 다시 계산.
    #   reprocessed_at 을 찍어 "이미 재처리함"으로 표시(다음 배치 제외).
    #   → 새 점수가 4점을 넘으면 보관기간이 7일↑로 자동 연장(= 승격).
    #   → 4점 미만이면 원래대로 1일 뒤 폐기(단 표시는 남음).
    # -------------------------------------------------------------
    def apply_reprocess(self, result_id, new_code, new_score, new_summary):
        # 새 점수에 맞는 보관 일수 계산 (save() 와 같은 규칙)
        expiry_days = None
        for threshold in sorted(DATA_RETENTION["auto_delete_days"].keys()):
        ...
        cursor.execute(
            """
            UPDATE results
            SET code = ?, ai_score = ?, ai_summary = ?,
                expires_at = ?, reprocessed_at = ?
            WHERE id = ?
            """,
            (new_code, new_score, new_summary, expires_at,
