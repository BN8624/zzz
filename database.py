...
class CodeDatabase:
    ...
    # -------------------------------------------------------------
    #  (제거) 배치 재처리 메서드 2개 ― get_reprocess_candidates / apply_reprocess
    #   폐지 사유: 3점대 코드를 보완해 4점으로 끌어올려도 모델 능력 자체가
    #   천장이라 실효가 없었고, 3.5 Flash(유료)에 의존해 무료 운영 원칙과도
    #   맞지 않았습니다. 이제 4점 미만은 즉시 폐기합니다.
    #   * reprocessed_at 컬럼은 기존 DB 호환을 위해 그대로 둡니다(미사용).
    # -------------------------------------------------------------
