...
#
#  *** 키 3종 분리 ***
#    - 요약   : PAID_API_KEY        (유료 AI Studio Flash-Lite, RPM 3k)
#    - 임베딩 : PAID_API_KEY        (유료 AI Studio embedding-2, RPM 3k)
...
#  15면 요약·임베딩(둘 다 RPM 3k) 한도 안에서 빠르게. 메모리도 가볍습니다.
CONCURRENCY = 15
#  이 개수만큼 모이면 DB 에 저장하고 메모리를 비웁니다.
BATCH = 100

embed_client = genai.Client(api_key=os.getenv("PAID_API_KEY"))                    # 임베딩(유료)
summary_client = genai.Client(api_key=os.getenv("PAID_API_KEY"))                 # 요약(유료, RPM 3k)
...
async def process_one(req, meta, sem):
    ...
    async with sem:
        await asyncio.sleep(0.1)  # 살짝만 분산(같은 키 동시 호출 완화)
        # 1) 요약 (버텍스 Flash-Lite)
        try:
