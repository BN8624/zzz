...
from config import GOOGLE_API_KEY, MODELS, NOTEBOOK_PATH, SELF_IMPROVE
...
class MistakeNotebook:
    ...
    def add_mistake(self, task, problem, lesson, correct_pattern=""):
        ...
        # 상한 관리: 오답노트가 너무 커지면 검색이 느려지므로 개수를 제한합니다.
        #  넘치면 "실제로 검색돼 쓰인 횟수(usage_count)가 낮은 것"부터 버립니다.
        #  (동점이면 오래된 것부터) → 아무도 안 찾는 쓸모없는 교훈이 자연 도태됩니다.
        self._prune()

        self.save()
        print(f"  [+] 실수 #{mistake['id']} 오답노트에 추가됨 (총 {len(self.data['mistakes'])}개)")
        return True

    # -------------------------------------------------------------
    #  오답노트 상한 관리(도태) ― 개수가 상한을 넘으면 쓸모 낮은 것부터 삭제.
    #   기준: usage_count 오름차순(적게 쓰인 것 먼저) → 같으면 added_at 오름차순(오래된 것 먼저).
    # -------------------------------------------------------------
    def _prune(self):
        limit = SELF_IMPROVE["max_mistakes"]
        items = self.data["mistakes"]
        if len(items) <= limit:
            return
        # 살아남을 우선순위가 높은 순(많이 쓰인 것·최신)으로 정렬해서 앞에서 limit개만 보존.
        items.sort(
            key=lambda m: (m.get("usage_count", 0), m.get("added_at", "")),
            reverse=True,
        )
        removed = len(items) - limit
        self.data["mistakes"] = items[:limit]
        print(f"  [-] 오답노트 상한({limit}) 초과 → 쓸모 낮은 {removed}개 도태")
