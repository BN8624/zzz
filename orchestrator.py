...
from agents import call_gemma_26b, call_gemma_31b, call_flash_lite
...
"""
    ...
    def record_lesson(self, idea, code, review, evaluation):
        ...
        # 이전 평가/검토에 이미 "약점"이 들어있으니, 그걸 재료로 한 줄 교훈을 뽑습니다.
        summary = evaluation.get("summary", "")
        prompt = f"""당신은 코드 실패 사례에서 교훈을 뽑아내는 분석가입니다.
...
# 할 일
이 실패에서 "다음에 같은 실수를 반복하지 않기 위한 핵심 교훈"을 뽑으세요.
- 이 작업에만 해당하는 게 아니라, 비슷한 작업에도 적용될 수 있게 일반화하세요.
- 반드시 한국어로, 한 문장으로만 쓰세요. (다른 설명·머리말 없이 문장 하나만)

예시) "외부 요청 코드는 타임아웃과 예외 처리를 반드시 포함할 것"
"""
        try:
            lesson = call_flash_lite(prompt)
