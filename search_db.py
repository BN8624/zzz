...
def search(query, top_k=5):
    ...
    conn = open_db()
    # 2) KNN 검색: 이 sqlite-vec 버전은 ORDER BY+LIMIT 대신 'k=?' 문법을 요구.
    #    먼저 KNN 으로 (rowid, distance) 를 뽑고, 그 rowid 들로 docs 메타를 가져옵니다.
    #    (JOIN 을 한 쿼리에 섞으면 k 제약이 깨지므로 두 단계로 분리)
    knn = conn.execute(
        """
    ...
    for rowid, dist in knn:
        d = conn.execute(
            "SELECT full_name, stars, description FROM docs WHERE id = ?",
            (rowid,),
        ).fetchone()
        if d:
            rows.append((d[0], d[1], d[2], dist))
    conn.close()
    return rows
